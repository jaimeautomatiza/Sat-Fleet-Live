...
const TLE_TTL         = 6 * 3600;    // 6 horas (Cache API)
...
const TLE_SINGLE_TTL = 6 * 3600;

async function fetchCelestrak(noradId) {
  const res = await fetch(`https://celestrak.com/NORAD/elements/gp.php?CATNR=${noradId}&FORMAT=json`, {
    headers: { 'User-Agent': 'SatFleetLive/3.0 (https://satfleetlive.com)' },
  });
...
async function handleTleSingle(request, env) {
  ...
  try {
    let data = await fetchCelestrak(noradId);
    
    if (!data.ok || !data.text.trim() || data.text.includes('No TLE found')) {
      data = await fetchCelestrak(noradId);
    }

    if (!data.ok || !data.text.trim() || data.text.includes('No TLE found')) {
      return new Response('Not found', { status: 404, headers: makeHeaders({ 'Content-Type': 'text/plain' }) });
    }

    try { await env.LAUNCHES_KV.put(kvKey, data.text, { expirationTtl: TLE_SINGLE_TTL }); } catch(e) {}
    return new Response(data.text, { status: 200, headers: makeHeaders({ 'Content-Type': 'application/json; charset=utf-8', 'X-Cache': 'MISS' }) });
  } catch(err) {
...
async function handleSatelliteInfo(request, env) {
  ...
  try {
    ...
    await env.LAUNCHES_KV.put(kvKeyLock, '1', { expirationTtl: 30 });
  } catch(e) {}
  ...
  try {
    const res = await fetch('https://onesignal.com/api/v1/notifications', {
      ...
      body: JSON.stringify({
        ...
        contents,
        data,
        url:               'https://satfleetlive.com/launches.html',
        large_icon:        'https://satfleetlive.com/images/logo.png',
        chrome_web_icon:   'https://satfleetlive.com/images/logo.png',
  ...
  ctx.waitUntil((async () => {
    ...
    if (cached?.results?.length && results.length) {
      ...
      for (const newL of results) {
        ...
        if (
          oldL?.net && newL.net &&
          new Date(newL.net).getTime() - new Date(oldL.net).getTime() > 3_600_000
        ) {
          const newDate = new Date(newL.net);
          ...
          notifPromises.push(sendOneSignalNotification(env,
            { en: '⏸️ Launch delayed',    es: '⏸️ Lanzamiento retrasado' },
            {
              en: `Mission ${newL.name} has been delayed. New target: ${dateStr}.`,
              es: `La misión ${newL.name} ha sido retrasada. Nueva fecha: ${dateStr}.`,
            },
            { launchId: newL.id, type: 'delay' }
          ));
        }
